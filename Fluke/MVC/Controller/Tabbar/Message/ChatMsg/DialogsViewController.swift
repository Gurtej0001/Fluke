...
//        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if chatDialog.lastMessageText == nil && chatDialog.lastMessageID != nil {
            cell.dialogLastMessage.text = "[Media]"
        }
        if let dateSend = chatDialog.lastMessageDate {
            cell.lastMessageDateLabel.text = setupDate(dateSend)
        } else if let dateUpdate = chatDialog.updatedAt {
        ...
        self.setUserDetails(userDetails: ChatManager.instance.storage.otherUser(with: chatDialog.id!), ivImageView: cell.ivImage, viewOnline: cell.viewOnline)
        
        return cell
    }
    
    private func setUserDetails(userDetails: QBUUser, ivImageView: UIImageView, viewOnline:UIView) {
        
        if let customDataString = convertToDictionary(text: userDetails.customData ?? "") {
            setImagefromURL(ivImage: ivImageView, strUrl: "\(customDataString["profileImage"] ?? "")")
        }
        else {
            setImagefromURL(ivImage: ivImageView, strUrl: nil)
        }
        
        let currentTimeInterval = Date().millisecondsSince1970
        let lastInterval = userDetails.lastRequestAt?.millisecondsSince1970 ?? 0

        if((currentTimeInterval - lastInterval) > 30){
            viewOnline.backgroundColor = UIColor.gray
        }
        else {
            viewOnline.backgroundColor = UIColor.green
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        chatViewController.otherUserName = cellModel.textLabelText
        chatViewController.dialogID = dialog.id
        chatViewController.chatdialog = dialog
    ...
    private func reloadContent() {
        ...
        let temp = chatManager.storage.dialogsSortByUpdatedAt()
        dialogs.removeAll()
        
        for details in temp {
            if(details.lastMessageText != nil) {
                dialogs.append(details)
            }
        }
...
extension DialogsViewController: QBChatDelegate {
    ...
    func chatDidReconnect() {
        SVProgressHUD.show(withStatus: "".localized)
        if QBChat.instance.isConnected == true {
            chatManager.updateStorage()
            SVProgressHUD.showSuccess(withStatus: "")
        }
