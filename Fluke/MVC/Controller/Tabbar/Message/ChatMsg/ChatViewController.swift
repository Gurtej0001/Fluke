...
struct ChatViewControllerConstant {
    ...
    static let messagePadding: CGFloat = 24.0
...
class ChatViewController: CallExtention {
    
    @IBOutlet private weak var lblName: UILabel!
    @IBOutlet private weak var lblIsOnline: UILabel!
    @IBOutlet private weak var ivImage: UIImageView!
    @IBOutlet private weak var viewOnline: UIView!
    ...
    private var messageForSend:QBChatMessage!
...
//        }
        ...
        inputToolbar.backgroundColor = UIColor.clear
        inputToolbar.tintColor = UIColor.clear
    ...
    override func viewDidLoad() {
        ...
        chatdialog.onJoinOccupant = { [weak self] userID in
            if userID == self?.senderID  {
                return
            }
            
            self?.setNewAttachmentMessgage()
            self?.onlineUsersIDs.insert(userID)
        }
        
        chatdialog.onLeaveOccupant = { [weak self] userID in
        ...
        if chatdialog.type == .private {
            ...
            chatdialog.onUserIsTyping = { [weak self] userID in
                guard let self = self else {
                    return
                }
                if userID == self.senderID  {
            ...
            chatdialog.onUserStoppedTyping = { [weak self] userID in
                if userID == self?.senderID  {
                    return
                }
                self?.typingUsers.remove(userID)
        ...
        let userDetails = chatManager.storage.otherUser(with: dialogID)
        self.lblName.text = otherUserName
        
        if let customDataString = convertToDictionary(text: userDetails.customData ?? "") {
            setImagefromURL(ivImage: self.ivImage, strUrl: "\(customDataString["profileImage"] ?? "")")
            self.getUserDetails(strUserId: "\(customDataString["userId"] ?? "")")
            
        }
        else {
            setImagefromURL(ivImage: self.ivImage, strUrl: nil)
        }
        
    }
        
    private func getUserDetails(strUserId: String) {
        
        UserModel.sharedUser.getUserDetailsFor(strId: strUserId, success: { (otherUserDetails) in
            self.setIsOnline(isOnline: otherUserDetails.isOnline)
        }) { (strError) in
    ...
    private func setIsOnline(isOnline: Bool) {
        
        DispatchQueue.main.async {
            if(isOnline == false) {
                self.viewOnline.backgroundColor = UIColor.gray
                self.lblIsOnline.text = "offline"
            }
            else {
                self.viewOnline.backgroundColor = UIColor.green
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        chatdialog.clearTypingStatusBlocks()
    }
    ...
    @IBAction func profilePic(_ sender: UIButton) {
        ...
        if let customDataString = convertToDictionary(text: userDetails.customData ?? "") {
            ...
            let otherUserProfileVC = storyboard.instantiateViewController(withIdentifier: "OtherUserProfileVC") as! OtherUserProfileVC
            otherUserProfileVC.userId = "\(customDataString["userId"] ?? "")"
            self.navigationController?.pushViewController(otherUserProfileVC, animated: true)
        }
    }
    
    @IBAction func didTapCall(_ sender: UIBarButtonItem) {
        ...
        //if(UserModel.sharedUser.strPlanId == "1") {
            ...
            if let customDataString = convertToDictionary(text: userDetails.customData ?? "") {
                isDNDUser = (customDataString["isDND"] as? Bool ?? false)
            }
            
            if(isDNDUser == true) {
                self.showAlert(strMsg: "\(self.lblName.text!) is not available for a call")
            }
            else {
                self.setCallwithUser(with: .video, chatDialog: chatdialog, otherUserId: userDetails.login!)
            }
//        }
//        else {
//            DispatchQueue.main.async {
...
//        }
    ...
    @IBAction func didTapMore(_ sender: UIBarButtonItem) {
        ...
        if let customDataString = convertToDictionary(text: userDetails.customData ?? "") {
            ...
            let otherUserId = "\(customDataString["userId"] ?? "")"
            ...
            userProfile.strId = otherUserId
            userProfile.strEmailId = ""
            userProfile.strUserDisplayName = chatdialog.name ?? ""
            userProfile.strUserFirstName = chatdialog.name ?? ""
            userProfile.strUserLastName = chatdialog.name ?? ""
            userProfile.strProfileImage = "\(customDataString["profileImage"] ?? "")"
    ...
    private func setupInputToolbar() {
        ...
        let accessoryImage = UIImage(named: "iconAdd")
        let normalImage = accessoryImage?.imageMasked(color: #colorLiteral(red: 0.4901960784, green: 0.4901960784, blue: 0.4901960784, alpha: 1))
        let highlightedImage = accessoryImage?.imageMasked(color: #colorLiteral(red: 0.4901960784, green: 0.4901960784, blue: 0.4901960784, alpha: 1))
        let accessorySize = CGSize(width: accessoryImage?.size.width ?? 64.0, height: 32.0)
        ...
        let accessorySendImage = UIImage(named: "iconSend")
        let normalSendImage = accessorySendImage?.imageMasked(color: #colorLiteral(red: 0.9597017169, green: 0.3549151421, blue: 0.6354215741, alpha: 1))
        let highlightedSendImage = accessorySendImage?.imageMasked(color: #colorLiteral(red: 0.9597017169, green: 0.3549151421, blue: 0.6354215741, alpha: 1))
        let accessorySendSize = CGSize(width: accessoryImage?.size.width ?? 64.0, height: 28.0)
    ...
    private func attributedString(forItem messageItem: QBChatMessage) -> NSAttributedString? {
        ...
        let font = UIFont(name: "Poppins-Regular", size: 15.0)
    ...
    private func statusImageForMessage(message: QBChatMessage) -> UIImage {
        ...
        if let readIDs = message.readIDs?.filter({ $0 != NSNumber(value: senderID) }),
            ...
            if #available(iOS 13.0, *) {
                return #imageLiteral(resourceName: "delivered").withTintColor(#colorLiteral(red: 0.9597017169, green: 0.3549151421, blue: 0.6354215741, alpha: 1))
            } else {
                return #imageLiteral(resourceName: "delivered").imageMasked(color: #colorLiteral(red: 0.9597017169, green: 0.3549151421, blue: 0.6354215741, alpha: 1))!
            }
        }
    ...
    private func didTapDelete() {
        ...
        let leaveAction = UIAlertAction(title: "SA_STR_DELETE".localized, style: .default) { (action:UIAlertAction) in
            ...
            guard let dialogID = self.chatdialog.id else {
                SVProgressHUD.dismiss()
                return
            }
            
            if self.chatdialog.type == .private {
                ...
                // group
                self.chatdialog.pullOccupantsIDs = [(NSNumber(value: currentUser.ID)).stringValue]
                
                let message = "\(currentUser.fullName) " + "SA_STR_USER_HAS_LEFT".localized
                // Notifies occupants that user left the dialog.
                self.chatManager.sendLeaveMessage(message, to: self.chatdialog, completion: { (error) in
                    if let error = error {
    ...
    private func setNewAttachmentMessgage() {
        
        messageForSend = QBChatMessage.markable()
        messageForSend.text = "[Media]"
        messageForSend.senderID = senderID
        messageForSend.dialogID = dialogID
        messageForSend.deliveredIDs = [(NSNumber(value: senderID))]
        messageForSend.readIDs = [(NSNumber(value: senderID))]
        messageForSend.dateSent = Date()
        messageForSend.customParameters["save_to_history"] = true
    }
    ...
    private func showPickerController(_ pickerController:UIImagePickerController?, sourceType: UIImagePickerController.SourceType, isImage: Bool?) {
        ...
        let showAllAssets: () -> Void = { [weak self]  in
            ...
            DispatchQueue.main.async {
                ...
                if(isImage == true) {
                    selectAssetsVC.mediaType = .image
                }//218
                else {
                    selectAssetsVC.mediaType = .video
                }
                ...
                selectAssetsVC.selectedAssetCompletion = { [weak self] selectedAsset in
                    ...
                    if let selectedAsset = selectedAsset {
                        ...
                        DispatchQueue.main.async {
                            ...
                            self!.attachmentCount = selectedAsset.count
                            
                            for details in selectedAsset {
                                
                                if(isImage == true) {
                                    self?.showAttachmentBar(with: details, attachmentType: .Image)
                                }
                                else {
                                    self?.showAttachmentBar(with: details, url: details.videoURL, attachmentType: .Video)
                                }
                            }
                        }
                    }
...
//        if(UserModel.sharedUser.strPlanId == "1") {
            ...
            if let messageText = currentlyComposedMessageText(), messageText.isEmpty == false {
                ...
                sendAttachmentMessage(withAttachmentMessage: messageForSend)
            }
...
//        }
    ...
    private func sendAttachmentMessage(withAttachmentMessage attachmentMessage: QBChatMessage) {
    ...
    private func sendMessage(message: QBChatMessage) {
        ...
        chatManager.send(message, to: chatdialog) { [weak self] (error) in
            if let error = error {
    ...
    private func forvardMessage() {
        guard let selectedIndexPathForMenu = selectedIndexPathForMenu else {
            return
        }
        
        let message = dataSource.messages[selectedIndexPathForMenu.row]
        ...
        if let dialogsSelection = storyboard.instantiateViewController(withIdentifier: "DialogsSelectionVC") as? DialogsSelectionVC {
            ...
            navVC.navigationBar.barTintColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let videoURL = info[UIImagePickerController.InfoKey.mediaURL.rawValue] as? URL {
            ...
            do {
                ...
                let photoAsset = PhotoAsset(phAsset: PHAsset(), image: thumbnail, videoURL: nil)
...
extension ChatViewController: InputToolbarDelegate {
    ...
    func didPressAccessoryButton(_ sender: UIButton) {
        ...
        #else
        alertController.addAction(UIAlertAction(title: "Picture from Camera", style: .default, handler: { (action) in
            self.showPickerController(self.pickerController, sourceType:.camera, isImage: true)
        }))
        
        alertController.addAction(UIAlertAction(title: "Video from Camera", style: .default, handler: { (action) in
            self.showPickerController(self.pickerController, sourceType:.camera, isImage: false)
        }))
        
        #endif
        
        alertController.addAction(UIAlertAction(title: "Picture from Photos", style: .default, handler: { (action) in
            self.showPickerController(nil, sourceType: .photoLibrary, isImage: true)
        }))
        
        alertController.addAction(UIAlertAction(title: "Video from Photos", style: .default, handler: { (action) in
            self.showPickerController(nil, sourceType: .photoLibrary, isImage: false)
        }))
...
extension ChatViewController: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        guard let selectedIndexPath = selectedIndexPathForMenu,
            let selectedCell = collectionView.cellForItem(at: selectedIndexPath) as? ChatCell else {
                return false
        }
        ...
        self.popVC = popVC
        popVC.actions = chatdialog.type == .private ? [.Forward] : [.Forward, .DeliveredTo, .ViewedBy]
...
extension ChatViewController: ChatCollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if message.senderID != senderID {
            ...
            if message.readIDs?.contains(NSNumber(value: senderID)) == false {
                chatManager.read(message, dialog: chatdialog, completion: nil)
            }
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let item = dataSource.messages[indexPath.row]
    ...
    private func collectionView(_ collectionView: ChatCollectionView,
        ...
        if cell is ChatIncomingCell, chatdialog.type != .private {
        ...
        if let attachmentCell = cell as? ChatAttachmentCell {
            ...
            if attachmentCell is ChatAttachmentIncomingCell, chatdialog.type != .private {
                let userName = topLabelAttributedString(forItem: item)?.string
            ...
            if(item.attachments?.count ?? 0 > 1) {
                attachmentCell.lblNumberOfMedia.isHidden = false
                attachmentCell.lblNumberOfMedia.text = "\(item.attachments!.count - 1) More"
            }
            else {
                attachmentCell.lblNumberOfMedia.isHidden = true
            }
...
extension ChatViewController: ChatCellDelegate {
    ...
    func chatCellDidTapContainer(_ cell: ChatCell) {
        if let attachmentCell = cell as? ChatAttachmentCell {
            ...
            let indexpath = collectionView.indexPath(for: attachmentCell)
            
            let item = dataSource.messages[indexpath?.row ?? 0]
            ...
            if attachment.type == "image" {
                ...
                fullPhotosVC.qbChatAttachment = item.attachments
...
extension ChatViewController: AttachmentBarDelegate {
    ...
    func attachmentBar(_ attachmentBar: AttachmentUploadBar, didUpLoadAttachment attachment: QBChatAttachment) {
        ...
        if(messageForSend == nil) {
            self.setNewAttachmentMessgage()
        }
        
        var tempMessage:[QBChatAttachment] = messageForSend.attachments ?? [QBChatAttachment]()
        tempMessage.append(attachment)
        messageForSend.attachments = tempMessage
        
        if(attachment.type == "image") {
            if(self.attachmentCount == tempMessage.count) {
                self.attachmentCount = 0
                self.sendAttachmentMessage(withAttachmentMessage: messageForSend)
                messageForSend = nil
                hideAttacnmentBar()
            }
        }
