...
class ChatManager: NSObject {
    ...
    func send(_ message: QBChatMessage, to dialog: QBChatDialog, completion: QBChatCompletionBlock?) {
        ...
        for ID in dialog.occupantIDs! {
            
            if(ID != NSNumber(value: QBManager.sharedManager.myUser.id)) {
                event.usersIDs = "\(ID)"
                break
            }
        }
        ...
        pushParameters["message"] = message.text
        pushParameters["type"] = "chat"
        pushParameters["title"] = QBManager.sharedManager.myUser.fullName
        pushParameters["chatDialogId"] = dialog.id
        pushParameters["userId"] = "\(QBManager.sharedManager.myUser.id)"
        
        pushParameters["data.userId"] = UserModel.sharedUser.strId
        pushParameters["data.dialogId"] = dialog.id
        pushParameters["data.title"] = QBManager.sharedManager.myUser.fullName
        pushParameters["data.body"] = message.text
        pushParameters["data.type"] = "chat"
