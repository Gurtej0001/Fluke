...
class LoginTableViewController: UITableViewController {
    ...
    private func signUp(fullName: String, login: String) {
        ...
        QBRequest.signUp(newUser, successBlock: { [weak self] response, user in
            ...
            self?.login(fullName: fullName, login: login)
                ...
                self?.handleError(response.error?.error, domain: QBErrorDomain.signUp)
        })
    }
    ...
    private func login(fullName: String, login: String, password: String = LoginConstant.defaultPassword) {
        ...
        QBRequest.logIn(withUserLogin: login,
                ...
                self?.handleError(response.error?.error, domain: QBErrorDomain.logIn)
                                ...
                                completion: { [weak self] error in
                                    if let error = error {
                                        if error._code == QBResponseStatusCode.unAuthorized.rawValue {
                                            ...
                                            self?.handleError(error, domain: QBErrorDomain.logIn)
                                        }
                                    } else {
                                        //did Login action
    ...
    // MARK: - Handle errors
    private func handleError(_ error: Error?, domain: QBErrorDomain) {
