...
//                as? SessionSettingsViewController
            ...
            print("[UsersViewController] UsersSegueConstant.call")
        ...
        if hasConnectivity() {
            ...
            CallPermissions.check(with: conferenceType) { granted in
                if granted {
                    ...
                    self.dataSource.ids(users: self.dataSource.selectedUsers, success: { (ids) in
                      
                        //Create new session
                        
                        let session = QBRTCClient.instance().createNewSession(withOpponents: ids, with: conferenceType)
