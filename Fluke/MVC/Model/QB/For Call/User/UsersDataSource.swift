...
class UsersDataSource: NSObject {
    ...
    func ids(users: [QBUUser], success:@escaping ([NSNumber]) -> Void, failed:@escaping (String) -> Void) {
        
        var result = [NSNumber]()
        
        for user in users {
            
            QBRequest.user(withLogin: user.login!, successBlock: { (response, userDetails) in
                print("------------------------")
                print(userDetails.id)
                print("------------------------")
                result.append(NSNumber(value: userDetails.id))
                success(result)
